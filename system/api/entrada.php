...
function adicionar($data)
{
    ...
    if (!isset($data["valor"]) || !is_numeric((int)$data["valor"])) {
        die(json_encode(array("success" => false, "message" => "Valor inválido!")));
    }

    if (!isset($data["data"]) || !DateTime::createFromFormat('d/m/Y', $data["data"])) {
        die(json_encode(array("success" => false, "message" => "Data inválida!")));
    }
    ...
    $edita = Database::connection()->prepare("INSERT INTO `ganhos` (`user_id`, `nome`, `valor`, `data`, `categoria_id`, `banco_id`, `observacoes`) VALUES (?, ?, ?, ?, ?, ?, ?);");
    ...
    ActivityLogger::add("Adicionou a entrada \"" . $data["nome"] . "\" no valor de " . (int)$data["valor"] . ".");

    die(json_encode(array("success" => true, "message" => "Entrada adicionada com sucesso!")));
}
